          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <FormField name="district" control={form.control} render={({ field }) => (
                  <FormItem><FormLabel>District</FormLabel><Select onValueChange={(value) => { field.onChange(value); form.setValue('taluka', ''); form.setValue('village', ''); }} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select District" /></SelectTrigger></FormControl><SelectContent>{Object.keys(locationData).map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>
              )} />
              <FormField name="taluka" control={form.control} render={({ field }) => (
                  <FormItem><FormLabel>Taluka</FormLabel><Select onValueChange={(value) => { field.onChange(value); form.setValue('village', ''); }} value={field.value} disabled={!district}><FormControl><SelectTrigger><SelectValue placeholder="Select Taluka" /></SelectTrigger></FormControl><SelectContent>{district && locationData[district as keyof typeof locationData] && Object.keys(locationData[district as keyof typeof locationData]).map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>
              )} />
